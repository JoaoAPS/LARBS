...
[ -z "$dotfilesrepo" ] && dotfilesrepo="https://github.com/JoaoAPS/dotfiles.git"
[ -z "$progsfile" ] && progsfile="https://raw.githubusercontent.com/JoaoAPS/LARBS/master/progs.csv"
[ -z "$aurhelper" ] && aurhelper="yay"
[ -z "$repobranch" ] && repobranch="no_os"
...
welcomemsg() { \
	...
	dialog --colors --title "Important Note!" --yes-label "I have an os!" --no-label "Cancel!" --yesno "You are doing a no-os installation, which means an operating system must already be installed." 8 70 || clear; exit;
...
usercheck() { \
	...
	dialog --colors --title "WARNING!" --msgbox "You must be logged in to perform a no-os installation. Create a user if needed."  6 50 &&
	clear; exit;
}
...
gitmakeinstall() {
	...
	sudo -u "$USER" git clone --depth 1 "$1" "$dir" >/dev/null 2>&1 || { cd "$dir" || return ; sudo -u "$USER" git pull --force origin master;}
	cd "$dir" || exit
	make >/dev/null 2>&1
...
putgitrepo() { # Downloads a gitrepo $1 and places the files in $2 only overwriting conflicts
	...
	dir=$(mktemp -d)
	[ ! -d "$2" ] && mkdir -p "$2"
	chown -R "$USER":wheel "$dir" "$2"
	sudo -u "$USER" git clone --recursive -b "$branch" --depth 1 "$1" "$dir" >/dev/null 2>&1
...
# Give warning if user does not exists.
usercheck
...
yes | sudo -u "$USER" $aurhelper -S libxft-bgra >/dev/null 2>&1

# Install the dotfiles in the user's home directory (as in https://www.atlassian.com/git/tutorials/dotfiles)
alias config='/usr/bin/git --git-dir=$HOME/.dotfiles/ --work-tree=home/$USER'
echo -e "# Access dotfiles git repo\nalias config='/usr/bin/git --git-dir=$HOME/.dotfiles/ --work-tree=$HOME'\n" | tee $HOME/.bashrc $HOME/.zrc > /dev/null
git clone --bare $dotfilesrepo $HOME/.dotfiles
config checkout -f > 2>&1
config config --local status.showUntrackedFiles no

rm -f "$HOME/README.md" "$HOME/LICENSE" "$HOME/FUNDING.yml" 2> /dev/null
# make git ignore deleted LICENSE & README.md files
config update-index --assume-unchanged "$HOME/README.md"
config update-index --assume-unchanged "$HOME/LICENSE"
